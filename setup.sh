...
if [[ -z "$API_NAME" ]]; then
    API_NAME="taiga-api.geniustree.ath.cx";
fi
...
docker run -d --name taiga-front -p 8210:80 -e API_NAME=$API_NAME --link taiga-back:taiga-back --volumes-from taiga-back ipedrazas/taiga-front
